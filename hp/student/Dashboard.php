...
<body class="layout-4">
    ...
    <div id="app">
        ...
        // User is logged in as student
              $selectQuery = "SELECT job_post.* 
FROM job_post 
LEFT JOIN job_request ON job_post.id = job_request.job_post_id and job_request.student_id = ".$_SESSION['user']['id']."
WHERE job_request.id IS NULL";

        $allJobPostResult =  mysqli_query($connection, $selectQuery);


        $companyQuery = "Select count(*) from company";
        $companyQueryResult = mysqli_query($connection, $companyQuery);
        $totalCompany = mysqli_fetch_array($companyQueryResult)[0];

        $totalAppliedJobQuery = "Select count(*) from job_request where student_id = " . $_SESSION["user"]["id"];
        $totalAppliedJobResult = mysqli_query($connection, $totalAppliedJobQuery);
        $totalAppliedJob = mysqli_fetch_array($totalAppliedJobResult)[0];

        $rejectStudentQuery = "Select * from job_request where is_hire = 0 and student_id = " . $_SESSION["user"]["id"];
...
";
        ...
        $jobPostQuery = "
  SELECT 
    MONTH(apply_date_time) AS month,
    MONTHNAME(apply_date_time) AS month_name,
    COUNT(*) AS total
  FROM job_request
  WHERE student_id = " . $_SESSION["user"]["id"] . "
  GROUP BY MONTH(apply_date_time)
  ORDER BY MONTH(apply_date_time)
";
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12">
                            <div class="card card-statistic-2">
                                ...
                                <div class="card-wrap">
                                    <div class="card-header">
                                        <h4>Total Applied Job</h4>
                                    </div>
                                    <div class="card-body">
                                        <?php echo $totalAppliedJob; ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <div class="card-header-action">
                                        <a class="btn btn-danger" href="/hp/apply_job_post/index.php">View More <i class="fas fa-chevron-right"></i></a>
                                    </div>
                                </div>
                                        ...
                                        <table class="table table-striped">
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="btn btn-primary" href="/hp/apply_job_post/form.php?id=<?php echo $row['id'] ?>">Apply</a>
                                                    </td>
                                                </tr>
                                            <?php
                                            }
