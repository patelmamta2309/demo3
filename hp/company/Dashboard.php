...
<body class="layout-4">
    ...
    <div id="app">
        ...
        $whereQuery = '';
        if ($type == 'Company') {
            $whereQuery = "Where company_id =" . $_SESSION['user']['id'];
        }
        $selectQuery = "SELECT * FROM `job_post` " . $whereQuery;

        $jobresult =  mysqli_query($connection, $selectQuery);


        $hireStudentQuery = "Select * from job_request where is_hire = 1 and company_id = " . $_SESSION["user"]["id"];
        $hireStudentQueryResult = mysqli_query($connection, $hireStudentQuery);
        $totalHireStudent = mysqli_num_rows($hireStudentQueryResult);

        $rejectStudentQuery = "Select * from job_request where is_hire = 0 and company_id = " . $_SESSION["user"]["id"];
        ...
        $jobpostQuery = "Select * from job_Post where company_id = " . $_SESSION["user"]["id"];
